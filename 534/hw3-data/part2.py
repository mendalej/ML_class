...
# my_train_data = my_train_data.astype(str)
# my_dev_data = my_dev_data.astype(str)
# my_test_data = my_test_data.astype(str)
...
# for col in cat_cats:
#     my_train_data[col] = my_train_data[col].astype(str)
# for col in num_cats:
#     my_train_data[col] = my_train_data[col].astype(float)

# for col in cat_cats:
#     my_dev_data[col] = my_dev_data[col].astype(str)
# for col in num_cats:
#     my_dev_data[col] = my_dev_data[col].astype(float)

# for col in cat_cats:
#     if col in my_test_data.columns:
#         my_test_data[col] = my_test_data[col].astype(str)

# for col in num_cats:
#     if col in my_test_data.columns:
#         my_test_data[col] = my_test_data[col].astype(float).fillna(0)

cat_cats = my_train_data.select_dtypes(include=['object']).columns
num_cats = my_train_data.select_dtypes(include=['number']).columns

my_train_data[cat_cats] = my_train_data[cat_cats].astype(str)
my_train_data[num_cats] = my_train_data[num_cats].astype(float)

my_dev_data[cat_cats] = my_dev_data[cat_cats].astype(str)
my_dev_data[num_cats] = my_dev_data[num_cats].astype(float)
...
dataframe.to_csv('prediction.test.2.csv', index=False)
